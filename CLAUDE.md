...
- `js/robot.js` — autonomous companion robot (homepage only). `position: absolute`. `mountRobot(el, { scale, palette, mood })` — `ROBOT_PALETTES` exports `violet` (default) + `blue`; `mood: 'sad'` swaps to a frown + worried brows. SVG is `width/height: 100%` so it fills its scaled wrapper. Two are mounted: default violet 1× happy + a blue 2× sad one (`#robot-2`).
...
## Session Log
- **2026-05-15**: Added a second companion robot. Parameterized `mountRobot` with `{ scale, palette, mood }` (was hardcoded violet/72px/happy). Added `VIOLET` + `BLUE` palettes, exported `ROBOT_PALETTES`. Mounted a blue, 2×, sad robot (`#robot-2` in `index.html`) next to the default violet one in `homepage.js` (desktop branch only). Bug fixed mid-task: enlarging the wrapper had no visual effect because the SVG carried hardcoded `width="72" height="72"` — changed to `width/height: 100%` so it scales with its container. Sad mood = downturned frown path + inner-raised "worried" eyebrow lines. Not visually QA'd in-browser this session (Joel eyeballing locally).
...
- _(older entries pruned — keeping last 5 sessions)_
