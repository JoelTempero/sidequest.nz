...
## Session Log
- **2026-07-23**: Restructured contacts into the `.panel-people` list — each person's details now sit under their island (Joel: email + phone under South Island; Harrison: email under North Island). Retired the standalone `.panel-direct` "Hit us up direct" block entirely. Retargeted the link styling AND the mobile 44px tap-target rule from `.panel-direct a` → `.panel-people a`, bumped the per-person gap for separation, and dropped the mobile two-column people grid (stacks full-width now so the long emails don't overflow a half-width column). This is the "two columns, one per person" resolution of the old open design question.
- **2026-07-22**: Added Harrison's email (`harrison@stirlingsolutions.co.nz`) to the (now-retired) `.panel-direct` block, grouped after Joel's email/phone.
